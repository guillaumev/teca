...
class ARC2_TurtleSerializer extends ARC2_RDFSerializer {
  ...
  function getTerm($v, $term = '', $qualifier = '') {
    ...
    /* literal */
    $quot = '"';
		// Patch by Valeria: make sure that $v['value'] is a string
    //if (preg_match('/\"/', $v['value'])) {
		if (is_string($v['value']) && preg_match('/\"/', $v['value'])) {
      $quot = "'";
    ...
    if ((strlen($quot) == 1) && is_string($v['value']) && preg_match('/[\x0d\x0a]/', $v['value'])) {
      $quot = $quot . $quot . $quot;
    }
